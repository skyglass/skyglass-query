...
public abstract class AbstractQueryFilter<T, F> extends AbstractBaseDataFilter<T, F> 
    ...
    public AbstractQueryFilter(Class<T> rootClazz, JunctionType junctionType, ICriteriaQueryBuilder<T, T> queryBuilder,
            IFilterRequest request) {
        this(rootClazz, junctionType, IJoinType.INNER, queryBuilder, request);
    }

    public AbstractQueryFilter(Class<T> rootClazz, JunctionType junctionType, IJoinType joinType, ICriteriaQueryBuilder<T, T> queryBuilder,
            IFilterRequest request) {
    ...
    private TypedQuery<T> createResultQuery() {
        return queryBuilder.createQuery(queryBuilder.generateQueryString());
    }
